...
void PSD_callback(const geometry_msgs::PointStamped &DPSD_msg)
{
    //printf("PSD_callback! \n");
    m_buf.lock();
    psdQueue.push(DPSD_msg);
    m_buf.unlock();
}
...
void vio_callback(const nav_msgs::Odometry::ConstPtr &pose_msg)
{
    ...
    {
        // 获取最老的GPS数据和其时间
        geometry_msgs::PointStamped DPSD_msg = psdQueue.front();
        double psd_t = DPSD_msg.header.stamp.toSec();
        // 10ms sync tolerance
        // +- 10ms的时间偏差
        printf("vio t: %f, gps t: %f \n", t, psd_t);
        // 10ms sync tolerance,PSD的更新速率更高，100HZ的话5ms的误差吧
        if(psd_t >= t - 0.005 && psd_t <= t + 0.005)
        {
            // gps的经纬度，海拔高度
            //printf("receive GPS with timestamp %f\n", DPSD_msg->header.stamp.toSec());
            double psd_x = DPSD_msg.point.x;  
            double psd_y = DPSD_msg.point.y;
            double psd_z = DPSD_msg.point.z;
            cout << "PSD.x:" << psd_x << endl;
            cout << "PSD.y:" << psd_y << endl;
            cout << "PSD.z:" << psd_z << endl;
            // 向globalEstimator中输入PSD数据
            globalEstimator.inputPSD(t, psd_x, psd_y, psd_z);
            psdQueue.pop();
            // PSD的位姿写入文本文件
            std::ofstream foutP("/home/robot/catkin_DPSDVinsFusion/src/VINS-Fusion/output/psd.csv", ios::app);//续写
    ...
    // 融合后的位姿写入文本文件
    std::ofstream foutC("/home/robot/catkin_DPSDVinsFusion/src/VINS-Fusion/output/vio_global.csv", ios::app);
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber sub_PSD = n.subscribe("/dpsdData", 100, PSD_callback);//pubpsd_node发布的psd消息话题
