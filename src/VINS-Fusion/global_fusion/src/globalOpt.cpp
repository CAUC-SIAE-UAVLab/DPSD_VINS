...
void GlobalOptimization::inputPSD(double t, double psd_x, double psd_y, double psd_z)
{
    ...
    printf("new psd: t: %f x: %f y: %f z:%f \n", t, tmp[0], tmp[1], tmp[2]);
...
void GlobalOptimization::optimize()
{
    while(true)
    {
        if(newPSD)
        {
            ...
            //增加了局部参数化约束，确保四元数始终是单位长度，始终表示合法的旋转
            ...
            for (iterVIO = localPoseMap.begin(); iterVIO != localPoseMap.end(); iterVIO++, i++)
            {
                ...
                if(iterVIONext != localPoseMap.end())
                {
                    ...
                    // 计算两帧VIO之间的相对差（略）......
                    //比的不是两帧的变化，而是两帧之间的变换矩阵的差值                  //VIO系统的输入是VIO两帧之间的变化
                ...
                {   //将PSD的坐标值传入                                     //PSD下一时刻得到的系统测量值
                    ceres::CostFunction* psd_function = PSDError::Create(iterPSD->second[0], iterPSD->second[1],iterPSD->second[2]);
                    //printf("inverse weight %f \n", iterPSD->second[3]);
                    problem.AddResidualBlock(psd_function, loss_function, t_array[i]);//t_array[i]表示优化初值
            ...
            for (int i = 0; i < length; i++, iter++)
            {
            	...
            	if(i == length - 1)
            	{
            	    ...
            	    //这里计算PSD和VIO之间的外参下次用于将VIO轨迹转换到PSD轨迹下
